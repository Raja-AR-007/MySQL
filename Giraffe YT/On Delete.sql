...
DROP TABLE branch_supplier;
                                            ...
                                            -- ON DELETE
-- ON DELETE useful in deleting a Foreign key. 
-- ON DELETE have two types they are ON DELETE SET NULL and ON DELETE CASCADE.
-- Both are extemely useful when we define foreign key relationship btw the tables.

                                       -- ON DELETE SET NULL 
-- In Foreign key If i delete a value it will be Automatically replaced by NULL value.
CREATE TABLE branch(
	branch_id INT PRIMARY KEY,
    branch_name VARCHAR(20),
    mgr_id INT,
...
WHERE emp_id=102;
									  ...
									  -- ON DELETE CASCADE
-- In Foreign key If i delete a value, it will totally delete the values.
CREATE TABLE branch_supplier(
	branch_id INT,
    supplier_name VARCHAR(40),
    supply_type VARCHAR(40),
